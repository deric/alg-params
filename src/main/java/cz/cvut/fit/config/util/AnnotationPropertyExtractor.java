package cz.cvut.fit.config.util;

import cz.cvut.fit.config.ui.PropertyTableEditor;
import cz.cvut.fit.config.ui.DefaultEditor;
import cz.cvut.fit.config.ui.IntegerTextFieldEditor;
import cz.cvut.fit.config.ui.DropDownEditor;
import cz.cvut.fit.config.ui.DefaultRenderer;
import cz.cvut.fit.config.ui.PropertyEditor;
import cz.cvut.fit.config.ui.PropertyRenderer;
import cz.cvut.fit.config.ui.CheckBoxEditor;
import cz.cvut.fit.config.ui.TextFieldEditor;
import cz.cvut.fit.config.ui.DoubleLabel;
import cz.cvut.fit.config.ui.DoubleTextFieldEditor;
import cz.cvut.fit.config.DefaultMapProperty;
import cz.cvut.fit.config.MutableProperty;
import cz.cvut.fit.config.DefaultListProperty;
import cz.cvut.fit.config.AbstractProperty;
import cz.cvut.fit.config.Sandbox;
import cz.cvut.fit.config.DefaultArrayProperty;
import cz.cvut.fit.config.DefaultProperty;
import cz.cvut.fit.config.ContextAwareConstraintValidatorFactory;
import cz.cvut.fit.config.ConfigurationException;
import cz.cvut.fit.config.DefaultComponentProperty;
import cz.cvut.fit.config.Property;
import cz.cvut.fit.config.PropertyState;
...
import cz.cvut.fit.config.context.Context;
import cz.cvut.fit.config.context.DefaultContext;
import cz.cvut.fit.config.context.DefaultPublishingContext;
...
public class AnnotationPropertyExtractor implements PropertyExtractor {
        ...
        for (Field field : fields) {
            if (isProperty(field)) {
                ...
                if (isComponentProperty(field.getType())) {
                    property = new DefaultComponentProperty(field.getAnnotation(cz.cvut.fit.config.annotations.Property.class), field, sandbox, validator);
                } else if (isArrayProperty(field.getType())) {
                    property = new DefaultArrayProperty(field.getAnnotation(cz.cvut.fit.config.annotations.Property.class), field, sandbox, validator);
                } else if (isListProperty(field.getType())) {
                    property = new DefaultListProperty(field.getAnnotation(cz.cvut.fit.config.annotations.Property.class), field, sandbox, validator);
                } else if (isMappedProperty(field.getType())) {
                    property = new DefaultMapProperty(field.getAnnotation(cz.cvut.fit.config.annotations.Property.class), field, sandbox, validator);
                } else {
                    property = new DefaultProperty(field.getAnnotation(cz.cvut.fit.config.annotations.Property.class), field, sandbox, validator);
                }
    ...
    public boolean isProperty(Field field) {
        return CollectionUtils.exists(Arrays.asList(field.getDeclaredAnnotations()), new Predicate() {
            ...
            public boolean evaluate(Object o) {
                return ((Annotation) o) instanceof cz.cvut.fit.config.annotations.Property;
            }
        });
    }
    ...
    private boolean isComponentProperty(Class<?> type) {
        return type.isAnnotationPresent(cz.cvut.fit.config.annotations.Component.class);
    }
}
